...
void main() {
	...
	vec3 normal_cam_raw0 = texelFetch(u_normal, ivec2(gl_FragCoord.xy), 0).xyz;
	float initial_depth = texelFetch(u_depth, ivec2(gl_FragCoord.xy), 0).r;
	if(initial_depth == 1.0 || dot(normal_cam_raw0, normal_cam_raw0) < 0.9) {
		out_light_1_accum = vec4(0.0);
		return;
	}

	vec3 normal_cam0 = normalize(normal_cam_raw0);
	vec3 dir_inc_cam0 = cam_dir_to_z1(gl_FragCoord.xy);

	fb_pos pos0 = fb_pos(uvec2(gl_FragCoord.xy), vec2(0.5), initial_depth);
	...
	vec4 extras_0_0 = texelFetch(u_extra_0, ivec2(pos0.texel), 0);
	vec4 extras_1_0 = texelFetch(u_extra_1, ivec2(pos0.texel), 0);
	...
	if(
		...
		accum_ratio *= exp(-(
			pow(
				length(cross(dir_inc_cam0, prev_dir_inc_cam)),
	...
	if(
		...
		pos0.z -= pos0.z / 10000.0;
		uint max_side = uint(max(frxu_size.x, frxu_size.y));
		fb_traversal_results results = traverse_fb(
			pos0, roughness_0, pos_cam0, dir_inc_cam0, normal_cam0,
			u_hi_depth, u_depth, u_win_normal,
			uint(max_side / 20)//uint(mix(max_side / 20, max_side / 10, (1.0 - roughness_0)))
		);

		for(uint i = 0u; i < min(MAX_TRAVERSAL_RESULTS_COUNT, results.results_count); ++i) {
			fb_traversal_result result = results.results[i];
			...
			vec3 dir_inc_cam = dir_inc_cam0;
			vec3 normal_cam = compute_normal(
				dir_inc_cam0, normal_cam0, vec2(pos0.texel), roughness_0, i
			);
			vec3 dir_cam = reflect(dir_inc_cam, normal_cam);
			//normal_cam = normal_cam0;

			vec3 normal_cam_raw = texelFetch(u_normal, ivec2(result.pos.texel), 0).xyz;
			float d0 = texelFetch(u_depth, ivec2(result.pos.texel), 0).r;

			bool pass = 
				dot(normal_cam_raw, normal_cam_raw) > 0.9 &&
				(
					result.code == TRAVERSAL_SUCCESS// ||
					//(result.code == TRAVERSAL_POSSIBLY_UNDER && abs(pow(result.pos.z, 0.01) - pow(d0, 0.01)) < 0.0000005)
				);
			...
			if(pass) {
				...
				normal_cam = normalize(normal_cam_raw);
				dir_inc_cam = dir_cam;
			}
			...
			if(frx_worldHasSkylight == 1) {
				...
				if(straigth) {
					vec3 normal_cam_1 = compute_normal(
						dir_inc_cam, normal_cam, pos.texel, roughness, 0
					);
					dir_cam = reflect(dir_inc_cam, normal_cam_1);
					...
					for(uint i = 0u; i < steps; ++i) {
						vec3 normal_cam_1 = compute_normal(
							dir_inc_cam, normal_cam, pos.texel, roughness, i
						);
						dir_cam = reflect(dir_inc_cam, normal_cam_1);
			...
			if(pass) {
				float cs = max(0.0001, dot(-dir_inc_cam, normal_cam));
				...
				float kD = 1.0 - kS;
				light_1 += (
					light + (kD * color + kS * (1.0 - roughness)) * cs * s
				) / float(results.results_count);
			}
			else {
				light_1 += s / float(results.results_count);
			}
		}
	}
