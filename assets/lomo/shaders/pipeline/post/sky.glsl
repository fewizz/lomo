...
const float earth_radius = 600000.0;
...
vec3 sky(vec3 dir, float sun_mul) {
	...
	vec3 color = sky(eye, layer(earth_radius, 8000.0), 0.005 / rgb) * 1.5;
	vec3 s = sky(eye, layer(earth_radius, 1200.0), 0.1 / rgb) * henyey_greenstein_phase_function(0.85, a) * vec3(4.0, 1.7, 0.3) * 2.0;
	s *= sun_mul;
...
vec3 end_sky(vec3 dir) {
	...
	return
		vec3(0.5, 0.1, 0.1) * abs(dir.y) +
		vec3(0.2, 0.1, 0.8) * abs(rotated_0.y) +
