...
vec3 sky(vec3 dir, float sun_mul) {
	...
	vec3 color = sky(eye, layer(earth_radius, 8000.0), 0.005 / rgb) * vec3(1.0, 2.0, 3.0) * 0.5;
	vec3 s = sky(eye, layer(earth_radius, 1200.0), 0.1 / rgb) * henyey_greenstein_phase_function(0.7, a) * vec3(3.0, 2.0, 1.0) * 1.0;
	s *= sun_mul;
