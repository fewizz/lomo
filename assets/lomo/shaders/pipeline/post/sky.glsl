...
vec3 sky(vec3 dir, float sun_mul) {
	...
	vec3 color = sky(eye, layer(earth_radius, 8000.0), 0.005 / rgb) * vec3(0.6, 1.3, 2.0);
	vec3 s = sky(eye, layer(earth_radius, 1200.0), 0.05 / rgb) * henyey_greenstein_phase_function(0.995, a) * vec3(16.0, 8.0, 2.0);
	s *= sun_mul;
