...
vec3 sky(vec3 dir, float sun_mul) {
	...
	vec3 color = sky(eye, layer(earth_radius, 8000.0), 0.01 / rgb) * 1.0;
	color.r *= 0.2;
	color.g *= 0.6;
	vec3 s = sky(eye, layer(earth_radius, 8000.0), vec3(0.0001)) * henyey_greenstein_phase_function(0.999, a) * 5.0;
